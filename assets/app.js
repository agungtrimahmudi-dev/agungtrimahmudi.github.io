...
const PROJECTS = [
  ...
  /* ---------- III. AI Chatbot UMKM ---------- */
  {
    ...
    id: {
      ...
      hook: 'Bot Telegram + Google Sheets + AI, dibangun sebagai studi kasus. Pemilik usaha yang mencobanya menilainya bagus, tapi usahanya berjalan sepenuhnya offline, jadi bot ini belum punya pekerjaan.',
      sections: [
        ...
        { h: 'Solusi', p: 'Saya bangun bot Telegram yang membaca stok dan harga langsung dari Google Sheets, jadi tidak ada data yang perlu disalin dua kali. Di atasnya ada tiga hal lagi: pengecekan stok dan harga, deteksi pembayaran yang mengabari admin begitu ada transfer masuk, dan mode admin supaya stok dan harga bisa diperbarui lewat chat tanpa membuka spreadsheet.\n\nTelegram saya pilih karena API-nya terbuka dan gratis. WhatsApp menuntut API berbayar atau pindah ke WA Business, dan untuk sebuah studi kasus biaya itu tidak masuk akal dikeluarkan hanya untuk membuktikan logikanya bekerja.' },
        { h: 'Hasil', p: 'Seluruh alurnya saya uji dari ujung ke ujung: cek stok, cek harga, deteksi pembayaran, dan mode admin, semuanya bekerja seperti yang dirancang. Teman saya ikut mencobanya sendiri dan menilainya bagus.\n\nMeski begitu, sampai sekarang dia belum memakainya. Usahanya berjalan sepenuhnya offline, pembeli datang langsung, dan dia memang tidak berniat melayani transaksi online maupun pengantaran. Bot yang menjawab pertanyaan stok lewat chat tidak punya pekerjaan di toko yang seluruh pembelinya berdiri di depan meja.\n\nSaya memilih tetap menampilkannya apa adanya. Yang saya buktikan di sini adalah alurnya bisa bekerja, bukan bahwa ada yang memakainya, dan dua hal itu tidak sama. Pelajaran yang saya bawa: memvalidasi bahwa sesuatu bisa dibangun jauh lebih mudah daripada memvalidasi bahwa seseorang benar-benar membutuhkannya, dan urutan yang benar adalah kebutuhan lebih dulu.' },
      ],
    },
    en: {
      title: 'SME AI Chatbot: An Automated Customer Service Case Study',
      hook: 'A Telegram bot + Google Sheets + AI, built as a case study. The shop owner who tried it rated it well, but his business runs entirely offline, so the bot has no job to do yet.',
      sections: [
        ...
        { h: 'Solution', p: 'I built a Telegram bot that reads stock and prices straight from Google Sheets, so no data has to be copied twice. On top of that sit three more things: stock and price lookups, payment detection that notifies the admin the moment a transfer arrives, and an admin mode so stock and prices can be updated over chat without opening the spreadsheet.\n\nI chose Telegram because its API is open and free. WhatsApp required either a paid API or a switch to WA Business, and for a case study that cost made no sense purely to prove the logic works.' },
        { h: 'Result', p: 'I tested the whole flow end to end: stock lookups, price lookups, payment detection, and admin mode all worked as designed. My friend tried it himself and rated it well.\n\nEven so, he has not put it to use. His business runs entirely offline, buyers come to him in person, and he has no intention of handling online orders or deliveries. A bot that answers stock questions over chat has no job to do in a shop where every buyer is already standing at the counter.\n\nI chose to show it exactly as it is. What I proved here is that the flow works, not that anyone uses it, and those are not the same thing. The lesson I took away: proving something can be built is far easier than proving someone actually needs it, and the correct order is need first.' },
      ],
    },
  },
